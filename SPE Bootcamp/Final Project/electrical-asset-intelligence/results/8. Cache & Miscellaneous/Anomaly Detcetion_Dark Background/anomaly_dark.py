...
def plot_anomaly_score_dark(scores, threshold=None, asset_type="asset", model_name="model", xlabel="Sample Index"):
    """
    Save a dark-background scatter plot of anomaly scores vs sample index.
    Points above threshold are red, others blue — similar style to the attached image.
    """
    scores = np.asarray(scores)
    if not os.path.exists('results'):
        os.makedirs('results')
    ...
    colors = ['red' if s > (threshold if threshold is not None else np.percentile(scores, 95)) else 'cornflowerblue' for s in scores]
    ax.scatter(np.arange(len(scores)), scores, c=colors, s=36, edgecolors='k', linewidths=0.2, alpha=0.9)
    if threshold is not None:
        ax.axhline(y=threshold, color='red', linestyle='--', linewidth=1)
    ax.set_title(f'{model_name} Anomaly Scores - {asset_type.capitalize()}')
    ax.set_xlabel(xlabel)
    ax.set_ylabel('Anomaly Score')
    ax.grid(True, color='gray', alpha=0.25)
    filename = f'results/anomaly_scores_{model_name.lower().replace(" ", "_")}_{asset_type}_dark.png'
    fig.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"Saved dark anomaly score plot to {filename}")
...
def detect_anomalies_for_asset(asset_type, data_path, contamination=0.05):
    ...
    try:
        ...
        # additional dark-themed scatter like attached
        plot_anomaly_score_dark(mse, threshold=threshold, asset_type=asset_type, model_name="LSTM", xlabel="Window Index")
    except Exception:
    ...
    try:
        ...
        # additional dark-themed scatter like attached
        plot_anomaly_score_dark(iso_scores, threshold=iso_threshold, asset_type=asset_type, model_name="Isolation Forest", xlabel="Sample Index")
    except Exception:
